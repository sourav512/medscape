...
var globalId = req.query.id; 
    ...
    request(`http://127.0.0.1:5000/?s1=${s1}&s2=${s2}&s3=${s3}&s4=${s4}&s5=${s5}`,async  function (error, response, body) {
      ...
      const message = `You are predicted to have ${newData.disease_predicted} disease with symptoms such as ${arr.toString()}`
       ...
       await symptomDoument.save()
        res.send(user); //Display the response on the website
      })
    }
